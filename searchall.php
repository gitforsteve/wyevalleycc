                ...
                if(strtotime($dt) > strtotime('-540 days')){
                    ...
                    while(!feof($f)){
                        ...
                        if( preg_match_all('/('.preg_quote($search,'/').')/i', $s, $matches)){
                            $found ++;
                            $searchok = true;
                            if($found === 1){$output.="<strong>Minutes for ".date("jS F Y",strtotime($dt))."</strong><br />";}
                            echo highlight_words($s,$search)."<br />";
                        }
                    }
                    fclose($f);
                    if($found > 0){$output.="<hr />";}
                }
            }
        }
        closedir($dh);
    }
    if(!$searchok){ echo "Sorry '".$search."' was not found<br><a href='minutes.php'>Back to Minutes</a>";}else{
        printf("<p>%s minutes contain your search</p>",$found);
        echo $output;
    }
?>
</div>
